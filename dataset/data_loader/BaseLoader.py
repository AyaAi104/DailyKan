...
class BaseLoader(Dataset):
    ...
    def __init__(self, dataset_name, raw_data_path, config_data, device=None):
        ...
        if self.do_preprocess:
            ...
            if self.config_data.PREPROCESS.CROP_FACE.BACKEND == 'INSIGHT':
                # InsightFace objects are not safely pickleable under multiprocessing spawn.
                # Each worker lazily initializes its own analyzer on first use.
                self.face_analyzer = None
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(pose_path):
            ...
            if pose.ndim != 2:
                raise ValueError(f"Pose file must be 2-D [T,3] or [3,T], got {pose.shape}: {pose_path}")
            if pose.shape[1] == 3:
    ...
    def preprocess_dataset(self, data_dirs, config_preprocess, begin, end):
        ...
        multi_process_quota = 2 if config_preprocess.CROP_FACE.BACKEND == "INSIGHT" else 8
        file_list_dict = self.multi_process_manager(
            data_dirs_split, config_preprocess, multi_process_quota=multi_process_quota)
    ...
    @staticmethod
    def _pose_from_insightface(face, previous_pose=None):
        """Return yaw, pitch, roll in degrees when InsightFace exposes pose."""
        if hasattr(face, "pose") and face.pose is not None:
            pose = np.asarray(face.pose, dtype=np.float32).reshape(-1)
            if pose.shape[0] >= 3:
                return pose[:3]
        if previous_pose is not None:
            return previous_pose.astype(np.float32)
    ...
    def crop_face_resize_with_pose(self, frames, use_face_detection, use_larger_box,
                                   larger_box_coef, width, height):
        """InsightFace crop/alignment plus per-frame yaw/pitch/roll extraction.

        Returns:
            resized_frames: [T,H,W,C]
            pose_angles: [T,3] in yaw/pitch/roll order
        """
            ...
            if use_face_detection:
                ...
                if len(faces) > 0:
                    ...
                    if hasattr(face, "kps") and face.kps is not None:
                        try:
                            ...
                            aligned_size = max(width, height)
                            crop = face_align.norm_crop(frame, landmark=face.kps, image_size=aligned_size)
                        except Exception:
                            crop = frame
                    else:
                        crop = frame
                ...
                if crop is frame:
                    if use_larger_box:
                        ...
                        face_box[0] = max(0, face_box[0] - (larger_box_coef - 1.0) / 2 * face_box[2])
                        face_box[1] = max(0, face_box[1] - (larger_box_coef - 1.0) / 2 * face_box[3])
                        face_box[2] = larger_box_coef * face_box[2]
                        face_box[3] = larger_box_coef * face_box[3]
                    face_box = np.asarray(face_box, dtype='int')
                    crop = frame[max(face_box[1], 0):min(face_box[1] + face_box[3], frame.shape[0]),
                                 max(face_box[0], 0):min(face_box[0] + face_box[2], frame.shape[1])]
            resized_frames[i] = cv2.resize(crop, (width, height), interpolation=cv2.INTER_AREA)

        return resized_frames, pose_angles
    ...
    def face_detection(self, frame, backend, use_larger_box=False, larger_box_coef=1.0):
        ...
        if backend == "HC":
            ...
            faces = self._get_face_analyzer().get(frame[:, :, :3].astype(np.uint8))
